...
const EthereumDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetchCoinPriceHistory("ethereum");
    ...
    <div className="mt-8">
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-8">
          <PriceChart currency={CoinName.Ethereum} priceData={priceData} />
          <Divider className="w-1/2"></Divider>
          <PriceTable currency={CoinName.Ethereum} priceData={priceData} />
        </div>
      )}
    </div>
  );
